...
2. **Prepara o Mac para o simulador ACE, para a leitura de mídia e para o ACE Audiovisual.** O simulador vai dentro do app e precisa de:
   ...
   **O Handy já vem configurado; as permissões continuam sendo suas.** Quando o `~/Library/Application Support/com.pais.handy/settings_store.json` ainda não existe, o instalador grava o padrão da TSA: português, atalho `option+espaço`, modelo `large-v3-turbo`, sem nenhuma chave de API. Se o arquivo já existe, ele não toca em nada. Permissão é outra história: o macOS só aceita Microfone e Acessibilidade pelo clique da pessoa, e nenhum script lê esse banco. Por isso a instalação termina com um passo guiado — abre as duas telas de Ajustes do Sistema, mostra os três passos e espera o Enter, com limite de 300 segundos (`TSA_PERM_TIMEOUT`). Sem terminal, ele só deixa o texto e nunca espera. O instalador jamais diz que conferiu a permissão, porque não tem como. O Handy é ferramenta de ditado, não faz parte da esteira de vídeo: se faltar, isso é aviso, não pendência que segura o simulador.
